...
function buildSystemStatus(env) {
  try {
    const storage = resolveStorageBackend(env);
    return {
      ok: true,
      status: "ok",
      service: "smart-care-sync-api",
      version: "2026-08-17",
      timestamp: Date.now(),
    ...
    return {
      ...
      storageReason: String(error && error.message ? error.message : error)
    };
  }
}

function buildHealthResponse(env) {
  const base = buildSystemStatus(env);
  return {
    ...base,
    api: "health",
    statusText: "ok"
  ...
  async fetch(req, env) {
    ...
    try {
      ...
      if (pathname === "/api/health") {
        return json(200, buildHealthResponse(env));
      }

      if (pathname === "/api/status") {
        return json(200, buildSystemStatus(env));
      }

      if (pathname === "/state") {
        if (req.method === "GET") {
          if (keyParam) {
            const syncKey = keyParam;
        ...
        if (req.method === "DELETE") {
          if (!keyParam) return json(400, { ok: false, error: "missing key" });
          const syncKey = keyParam;
          const kvKey = `smart-care:${syncKey}`;
          const result = await deleteValue(env, kvKey, syncKey);
          return json(200, { ok: true, deleted: true, key: syncKey, storage: result.backend, storageReason: result.reason });
        }
      }

      if (pathname === "/api/sync" || pathname === "/api/users") {
        const syncKey = keyParam || "smart-care-default";
        const kvKey = `smart-care:${syncKey}`;

        if (req.method === "GET") {
          const { value, backend, reason } = await loadStoredValue(env, kvKey, syncKey);
          return json(200, { ok: true, key: syncKey, value: value || null, storage: backend, storageReason: reason });
        }
